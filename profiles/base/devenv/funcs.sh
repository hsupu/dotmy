...
function enable_nvm() {
    ...
    source_or_warn "${NVM_DIR}/nvm.sh"
    if [[ $(uname -s) == "Darwin" ]]; then
        # brew
        source_or_warn "$(brew --prefix nvm)/etc/bash_completion.d/nvm"
    else
        source_or_warn "${NVM_DIR}/bash_completion"
    fi
...
function enable_rvm() {
    ...
    source_or_warn "${RVM_DIR}/scripts/rvm"
}
